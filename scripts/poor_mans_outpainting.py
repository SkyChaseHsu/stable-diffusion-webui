...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        inpainting_fill = gr.Radio(label='Masked content',
                                   ...
                                   type="index", elem_id=self.elem_id("inpainting_fill"))
        direction = gr.CheckboxGroup(label="Outpainting direction", choices=['left', 'right', 'up', 'down'],
                                     value=['left', 'right', 'up', 'down'], elem_id=self.elem_id("direction"))
    ...
    def run(self, p, pixels, mask_blur, inpainting_fill, direction):
        ...
        if opts.samples_save:
            images.save_image(combined_image, p.outpath_samples, "", initial_seed, p.prompt, opts.samples_format,
