...
def apply_optimizations():
    ...
    can_use_sdp = hasattr(torch.nn.functional, "scaled_dot_product_attention") and callable(
        getattr(torch.nn.functional, "scaled_dot_product_attention"))  # not everyone has torch 2.x to use sdp
    ...
    6, 0) <= torch.cuda.get_device_capability(shared.device) <= (9, 0)):
        ...
        optimization_method = 'V1'
            ...
            cmd_opts.opt_split_attention_invokeai or not cmd_opts.opt_split_attention and not torch.cuda.is_available()):
